...
export const Select: FC<ISelect> = ({ items, onDataChange, clearTrigger }) => {
  ...
  const [selectValue, setSelectValue] = useState<string>(items[0].label);
  ...
  return (
    <div
      ...
      ref={(ref) => (refClick.current = ref as Element)}
    >
      <div className={clsx(styles.Select_header)} onClick={handleOpen}>
        ...
        <div
          className={clsx(styles.Select_header_icon, isOpen && styles.active)}
        >
          <IconArrowSmall />
        </div>
      </div>
                ...
                {itemsSearch.map((e, i) => (
                  <div
                    className={clsx(
                      ...
                      indexActiveElem === i && styles.active
                    )}
                    onClick={() => {
                      handleOpen();
